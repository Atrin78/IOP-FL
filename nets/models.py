...
class Attention_block(nn.Module):
    def __init__(self,F_g,F_l,F_int, name):
        ...
        self.W_g = nn.Sequential(
            Conv2D(F_g, F_int, name=name+'_g', kernel_size=1,stride=1,padding=0,bias=True),
            nn.BatchNorm2d(F_int)
            )
        
        self.W_x = nn.Sequential(
            Conv2D(F_l, F_int, name=name+'_x', kernel_size=1,stride=1,padding=0,bias=True),
            nn.BatchNorm2d(F_int)
        )

        self.psi = nn.Sequential(
            Conv2D(F_int, 1, name=name+'_psi', kernel_size=1,stride=1,padding=0,bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
